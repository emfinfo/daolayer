...
public interface JpaDaoAPI {
   ...
   * une liste de valeurs de paramètres pour la clause de recherche WHERE.<br>
   * <br>
   * Si on doit travailler sur une seule entité, la méthode getList(Search
   * search) est plus facile d'utilisation.<br>
   * Quant à getList(String jpql, Search2 search), elle permet de préparer 
   * plus facilement une requête JPQL même avec des jointures 
   ...
   * @param maxResults une limitation des résultats si plus grand que zéro
   * 
   * @return une liste d'objets de l'entité spécifiée
   */
  //<E> List<E> getList(String jpql, Object[] params, int maxResults);
  ...
  /**
   * Pour la classe-entité spécifiée, récupère une liste d'objets non triés.
   *
   * @param cl  une classe entité managée par JPA
   * @return une liste d'objets non triés de la classe-entité spécifiée
   */
  // <E> List<E> getList(Class<?> cl);
